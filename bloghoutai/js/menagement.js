...
$('#queren').on('click',function(e){
    ...
    // alert('sdsg')
            ...
            $.cookie("audit",encodeURIComponent($('#Span').html()));
...
data ={
  ...
  Audit:decodeURIComponent($.cookie().audit)
  }
  console.log(data);
...
$.ajax({
  ...
  success: function (response) {
    $.get('sele.php',function(text){
      ...
      $('.dom').find('.val').eq(4).html(decodeURIComponent($.cookie().Role));
      $('.dom').find('.val').eq(5).html(decodeURIComponent(json[i].date));
      $('.dom').find('.val').eq(6).html(decodeURIComponent($.cookie().audit));
      // console.log($('.dom').find('.val'));
...
$.get('sele.php',function(text){
    ...
    $.each(json,function(index,value){
        ...
        aa[index] = value.id+','+value.name+','+value.phone+','+value.email+','+value.role+','+value.date+','+value.audit;
        $('tbody').append('<tr class="tr"><td class="ID allChoose"><span class = "allchoose"><img src="img/choose.png" alt=""></span>'+'<span class = "choose"><img src="img/choose.png" alt=""></span></td>'+'<td class="col-xs-1 val id"></td>'+'<td class="col-xs-1 val"></td>'+'<td class="col-xs-2 val"></td>' +' <td class="col-xs-1 val"></td>' +'<td class="val"></td>' + ' <td class="col-xs-2 val"></td>'+ '<td class="col-xs-2 val"></td>'+'<td class="editor col-xs-1 mid"><span id ="one" class="one" data-toggle="modal" data-target="#mymodal">编辑</span>'+'<span id ="two" class="two">删除</span></td></tr>'+'<br/>');
    })
    $('.val').css({
        'overflow':'hidden',
        'wordWrap':'break-word'
     ...
     $('#check').click(function(){
         ...
         $.get('sele.php',function(text){
            ...
            $.each(json,function(index,value){ 
                ...
                if($('#checkname').val() == value.name){
                   
                    $('.checkTable').append('<div class="checked"><div class="ID allChoose first"><span class = "allchoose"></span>'+'<span class = "choose"></span></div>'+'<div class="col-xs-1 val id">'+value.id+'</div>'+'<div class="col-xs-1 val">'+value.name+'</div>'+'<div class="col-xs-2 val">'+value.phone+'</div>' +' <div class="col-xs-1 val">'+value.email+'</div>' +'<div class="val width" style="width:210px">'+value.role+'</div>' + '<div class="col-xs-2 val">'+value.date+'</div>'+ '<div class="col-xs-2 val">'+value.audit+'</div>'+'<div class="editor col-xs-1 mid"></div>');
                }else if($('#checkPhone').val() == value.phone){
                   
                    $('.checkTable').append('<div class="checked"><div class="ID allChoose first"><span class = "allchoose"></span>'+'<span class = "choose"></span></div>'+'<div class="col-xs-1 val id">'+value.id+'</div>'+'<div class="col-xs-1 val">'+value.name+'</div>'+'<div class="col-xs-2 val">'+value.phone+'</div>' +' <div class="col-xs-1 val">'+value.email+'</div>' +'<div class="val width" style="width:210px">'+value.role+'</div>' + '<div class="col-xs-2 val">'+value.date+'</div>'+ '<div class="col-xs-2 val">'+value.audit+'</div>'+'<div class="editor col-xs-1 mid"></div>');
                }if($('#checkMail').val() == value.email){
                   
                    $('.checkTable').append('<div class="checked"><div class="ID allChoose first"><span class = "allchoose"></span>'+'<span class = "choose"></span></div>'+'<div class="col-xs-1 val id">'+value.id+'</div>'+'<div class="col-xs-1 val">'+value.name+'</div>'+'<div class="col-xs-2 val">'+value.phone+'</div>' +' <div class="col-xs-1 val">'+value.email+'</div>' +'<div class="val width" style="width:210px">'+value.role+'</div>' + '<div class="col-xs-2 val">'+value.date+'</div>'+ '<div class="col-xs-2 val">'+value.audit+'</div>'+'<div class="editor col-xs-1 mid"></div>');
                }if($('#checkRole').val() == value.role){
                ...
                if($('#name').val() == ''){
                    alert('未填写内容');
                        ...
                        $.get('sele.php',function(text){
                            ...
                            $('tbody').prepend('<tr class="tr"><td class="ID allChoose"><span class = "allchoose"><img src="img/choose.png" alt=""></span>'+'<span class = "choose"><img src="img/choose.png" alt=""></span></td>'+'<td class="col-xs-1 val id"></td>'+'<td class="col-xs-1 val"></td>' +'<td class="col-xs-2 val"></td>' +' <td class="col-xs-1 val"></td>' +'<td class="val"></td>' + ' <td class="col-xs-2 val"></td>'+ '<td class="col-xs-2 val"></td>'+'<td class="editor col-xs-1 mid"><span id ="one" class="one" data-toggle="modal" data-target="#mymodal">编辑</span>'+'<span id ="two" class="two">删除</span></td></tr>'+'<br/>');
